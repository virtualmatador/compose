...
bool compose::need_stop() const
{
    return stop_;
}

void compose::request_reload()
{
    reload_ = true;
}

void compose::request_stop()
{
    stop_ = true;
}
...
void compose::run(const std::chrono::milliseconds& nap_time)
{
    ...
    if (!pipe_path_.empty())
    {
        ...
        if (mkfifo(pipe_path_.c_str(), S_IROTH | S_IWOTH | S_IRUSR | S_IWUSR) != 0)
        {
            throw std::runtime_error("create pipe");
        }
        descriptor.reset(
            new int{ open(pipe_path_.c_str(), O_RDONLY | O_NONBLOCK) });
        if (*descriptor == -1)
        {
